...
{
    Install-ContainerHost
    # Remove default config file to unblock SF
    Write-Output "Deleting default docker configuration file"
    rm "$($env:ProgramData)\docker\config\daemon.json"
    # Restart SF to re-apply configurations
    restart-service FabricHostSvc
    restart-service ServiceFabricNodeBootstrapAgent
}
catch 
{
    Write-Error $_
